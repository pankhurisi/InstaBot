from paralleldots.config import set_api_key, get_api_key
import requests
import json


def get_parallel_dots_keyword(text, api_key_generated):
    print text
    print type(text)
    # paralleldot api key generated by me
    set_api_key("a95db2b0b5b9476fa77ceace841c34f1")
    # api key is being checked
    if not api_key_generated == None:
        if type(text) != str:
            print "Input should be string"
        elif text == "":
            print "Input cannot be empty"
            exit()
        else:
            parallel_dots_url = "http://paralleldots.com/keywords"
            user_info = requests.post(parallel_dots_url, params={"api_key_generated": api_key_generated, "q": text})
            # checking the status code
            if user_info.status_code != 200:
                print ""
            user_info = {"keywords": json.loads(user_info.content)}
            print user_info['keywords']
    else:
        print "The api key does not exist "
